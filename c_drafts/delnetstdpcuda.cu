											 ...
											 FLOAT_T synmax)
{
	...
	for (i=index; i < n; i += stride) {
		for (j=0; j < nums_in[i]; j++) {
			synapses[offsets[i]+j] = synapses[offsets[i]+j] +
					dt * (a_post * trace_pre[offsets[i]+j] * spike_post[i] -
						  a_pre * trace_post[i] * spike_pre[offsets[i]+j]);
			synapses[offsets[i]+j] =
				synapses[offsets[i]+j] < 0.0 ? 0.0 : synapses[offsets[i]+j];
			synapses[offsets[i]+j] =
				synapses[offsets[i]+j] > synmax ? synmax : synapses[offsets[i]+j];
		}
	}
}


static inline void synapse_strength_update_cuda(IDX_T n,
										   		IDX_T n_inputs,
										   		IDX_T *offsets,
										   		IDX_T *nums_in,
										   		FLOAT_T *trace_pre,
										   		...
										   		FLOAT_T synmax)
{
	...
	IDX_T *d_offsets=0, *d_nums_in=0;
	FLOAT_T *d_spike_pre=0, *d_trace_pre=0,
			*d_spike_post=0, *d_trace_post=0, *d_synapses=0;

	/* move data to GPU */
	cudaMemcpy(d_offsets, offsets, n*sizeof(IDX_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_nums_in, nums_in, n*sizeof(IDX_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_spike_post, spike_post, n*sizeof(FLOAT_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_trace_post, trace_post, n*sizeof(FLOAT_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_synapses, synapses, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_spike_pre, spike_pre, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyHostToDevice);
	cudaMemcpy(d_trace_pre, trace_pre, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyHostToDevice);

	synapse_strength_kernel_cuda<<<numblocks, TPB>>>(n,
											 		 d_offsets,
											 		 d_nums_in,
											 		 d_trace_pre,
											 		 d_trace_post,
											 		 ...
											 		 synmax);
	/* move data back from GPU to main memory */
	cudaMemcpy(offsets, d_offsets, n*sizeof(IDX_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(nums_in, d_nums_in, n*sizeof(IDX_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(spike_post, d_spike_post, n*sizeof(FLOAT_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(trace_post, d_trace_post, n*sizeof(FLOAT_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(synapses, d_synapses, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(spike_pre, d_spike_pre, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyDeviceToHost);
	cudaMemcpy(trace_pre, d_trace_pre, n_inputs*sizeof(FLOAT_T),
			cudaMemcpyDeviceToHost);
}
...
int main(int argc, char *argv[])
{
	...
	unsigned int cum_in = 0, exc_offset;
	for (i=0; i<n_exc; i++) {
